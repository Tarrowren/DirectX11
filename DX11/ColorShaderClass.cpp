...
bool ColorShaderClass::Initialize(ID3D11Device* device, HWND hwnd) {
	...
	result = InitializeShader(device, hwnd, (WCHAR*)L"ColorVertexShader.hlsl", (WCHAR*)L"ColorPixelShader.hlsl");
	if (!result) {
		return false;
	}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename) {
	...
	if (FAILED(result)) {
		if (errorMessage) {
			...
			MessageBox(hwnd, vsFilename, L"顶点着色器文件丢失", MB_OK);
		}
	...
	if (FAILED(result)) {
		if (errorMessage) {
			...
			MessageBox(hwnd, psFilename, L"像素着色器文件丢失", MB_OK);
		}
